...
The naive solution — `asyncio.run(coro)` per call — destroys the loop after each call, closing the underlying transport. MCP's `ClientSession` would need to reconnect on every `call_tool`, which is expensive and breaks server-side state.
...
Key methods — `_async_runner.py`:
- `__init__` — starts daemon thread + `loop.run_forever()` — line 17
- `run(coro, timeout)` — submits and blocks — line 37
- `close()` — `loop.stop()` + `thread.join()` — line 51
...
This means `tool_json_list` — `agent_tools.py:291` — calls `.model_json_schema()` on the generated class and the LLM receives the **actual schema** from the server.
...
| Field | Source |
|---|---|
| `content` | All MCP `"text"` blocks joined; or UTCP `text`/`content` field |
| `is_error` | MCP `CallToolResult.isError`; or UTCP `is_error` key |
...
Mirrors MCPToolBox. The main difference is that `UtcpClient.create()` accepts a `UtcpClientConfig` and manages provider registration internally. Tools are retrieved via `client.get_tools()` — `utcp.py:73`.

## OPM integration

Both classes are registered as entry points under `opm.agents.toolbox` — `pyproject.toml:24`. OPM instantiates them with `MyToolBox(config={...})` — no bus argument at construction. `bind(bus)` is called separately if the toolbox needs messagebus access.
