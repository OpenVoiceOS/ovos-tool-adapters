...
|---|---|---|---|---|
| `transport` | str | yes | — | `"stdio"` \| `"sse"` \| `"http"` |
| `command` | str | stdio | — | Executable to launch, e.g. `"uvx"`, `"python"` |
...
| `url` | str | sse/http | — | Full URL including scheme, host, port, path |
...
| Key | Type | Required | Default | Description |
|---|---|---|---|---|
| `utcp_config` | dict | yes | `{}` | Passed as `UtcpClientConfig(**utcp_config)` — see UTCP docs for keys |
